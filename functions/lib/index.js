...
exports.acceptFriendRequest = exports.updateWearCount = exports.generateAvatar = exports.generateOutfitRecommendations = exports.analyzeBodyShape = exports.analyzeClosetItem = void 0;
...
exports.analyzeClosetItem = functions.https.onCall(async (data, context) => {
    var _a, _b, _c, _d;
    if (!context.auth) {
...
Be accurate and specific. For hijab_friendly, return true if the item provides modest coverage (long sleeves, high neck, loose fit) or false if it's revealing. Return ONLY the JSON.`;
    try {
        ...
        const content = (_d = (_c = (_b = aiData.choices) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.message) === null || _d === void 0 ? void 0 : _d.content;
        if (!content) {
...
exports.analyzeBodyShape = functions.https.onCall(async (data, context) => {
    var _a, _b, _c;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    const ANTHROPIC_API_KEY = (_a = functions.config().anthropic) === null || _a === void 0 ? void 0 : _a.api_key;
...
Be as accurate as possible with measurements and provide helpful styling advice.`;
        ...
        const aiData = await response.json();
        const content = (_c = (_b = aiData.content) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.text;
        if (!content) {
            throw new functions.https.HttpsError('internal', 'No content in AI response');
        }
...
exports.generateOutfitRecommendations = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const closetItems = closetSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
        // Get body scan data
        ...
        const filteredItems = closetItems.filter((item) => {
            if (profile.isVeiled && !item.hijabFriendly)
                return false;
            if (item.suitableOccasions && item.suitableOccasions.length > 0) {
                return item.suitableOccasions.some((occ) => occ.toLowerCase().includes(occasion.toLowerCase()) ||
...
- Body shape: ${(bodyScan === null || bodyScan === void 0 ? void 0 : bodyScan.bodyShape) || 'Not specified'}
...
[
  {
    ...
    "occasion_fit": "Why it's perfect for ${occasion}"
        ...
        const aiData = await response.json();
        const aiContent = (_c = (_b = aiData.content) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.text;
        const cleanContent = aiContent.replace(/```json\n?|\n?```/g, '').trim();
        const recommendations = JSON.parse(cleanContent);
        // Enrich with full item details
        const enrichedOutfits = recommendations.map((outfit) => {
            const outfitItems = outfit.items.map((itemId) => filteredItems.find((item) => item.id === itemId)).filter(Boolean);
            return Object.assign(Object.assign({}, outfit), { items: outfitItems, total_items: outfitItems.length });
        });
        return {
            recommendations: enrichedOutfits,
            userPreferences: {
                isVeiled: profile.isVeiled,
                bodyShape: bodyScan === null || bodyScan === void 0 ? void 0 : bodyScan.bodyShape
            },
...
// Generate Avatar
exports.generateAvatar = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
        ...
        const analysisPrompt = `Analyze these body scan photos and extract the following information in JSON format:
    {
      ...
      "body_proportions": {
        ...
        const RPM_API_KEY = (_b = functions.config().readyplayerme) === null || _b === void 0 ? void 0 : _b.api_key;
        if (!RPM_API_KEY) {
...
// Update Wear Count
exports.updateWearCount = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userId = context.auth.uid;
